...
This package owns the DFS sample agent source for BigMaze:
[SampleDfsAgentBob.java](src/main/java/org/maze/scenarios/bigmaze/agents/SampleDfsAgentBob.java).

Start the server with this scenario package:

```shell
./gradlew runMase --args="--scenario scenarios/bigmaze"
```

In another terminal, run the agent:

```shell
./gradlew runBigMazeBobAgent
```
